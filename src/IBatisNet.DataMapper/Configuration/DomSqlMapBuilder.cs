...
namespace IBatisNet.DataMapper.Configuration {
  ...
  public class DomSqlMapBuilder {
    ...
    private const string CONFIG_XML_NAMESPACE = "http://mybatis.org/schema/mybatis-config";
    private const string MAPPER_XML_NAMESPACE = "http://mybatis.org/schema/mybatis-mapper";
    ...
    private ISqlMapper Build(XmlDocument document, DataSource dataSource,
      ...
      try {
        ...
        throw new ConfigurationErrorsException(_configScope.ErrorContext.ToString(), e);
      }
    }
    ...
    private void ValidateSchema(XmlNode section, string schemaFileName) {
      ...
      try {
        ...
        if (xsdFile == null) {
          ...
          throw new ConfigurationErrorsException("Unable to locate embedded resource [IBatisNet.DataMapper." + schemaFileName + "]. If you are building from source, verfiy the file is marked as an embedded resource.");
        }
        ...
        if (!_configScope.IsXmlValid) {
          throw new ConfigurationErrorsException("Invalid SqlMap.config document. cause :" + _configScope.ErrorContext.Resource);
        }
      } finally {
    ...
    private void Initialize() {
      ...
      if (nodeDataSource == null) {
        if (_configScope.IsCallFromDao == false) {
          throw new ConfigurationErrorsException("There's no dataSource tag in SqlMap.config.");
    ...
    private void ProcessSqlStatement(IStatement statement) {
      ...
      if (statement.ExtendStatement.Length > 0) {
        ...
        if (supperStatementNode != null) {
          ...
          throw new ConfigurationErrorsException("Unable to find extend statement named '" + statement.ExtendStatement + "' on statement '" + statement.Id + "'.'");
        }
      }
      ...
      for (int i = 0; i < count; i++) {
        ...
        if ((child.NodeType == XmlNodeType.CDATA) || (child.NodeType == XmlNodeType.Text)) {
          ...
          if (includeNode == null) {
            ...
            if (includeNode == null) {
              throw new ConfigurationErrorsException("Could not find SQL tag to include with refid '" + refid + "'");
            }
          }
        ...
        if (parameterMap.ExtendMap.Length > 0) {
          ...
          if (_configScope.SqlMapper.ParameterMaps.Contains(parameterMap.ExtendMap) == false) {
            ...
            if (superNode != null) {
              ...
              throw new ConfigurationErrorsException("In mapping file '" + _configScope.SqlMapNamespace + "' the parameterMap '" + parameterMap.Id + "' can not resolve extends attribute '" + parameterMap.ExtendMap + "'");
            }
          } else {
            superMap = _configScope.SqlMapper.GetParameterMap(parameterMap.ExtendMap);
          }
        ...
        if (resultMap.ExtendMap != null && resultMap.ExtendMap.Length > 0) {
          ...
          if (_configScope.SqlMapper.ResultMaps.Contains(resultMap.ExtendMap) == false) {
            ...
            if (superNode != null) {
              ...
              throw new ConfigurationErrorsException("In mapping file '" + _configScope.SqlMapNamespace + "' the resultMap '" + resultMap.Id + "' can not resolve extends attribute '" + resultMap.ExtendMap + "'");
            }
          } else {
            superMap = _configScope.SqlMapper.GetResultMap(resultMap.ExtendMap);
          }
